...
class XChaCha20Cipher:
    ...
    Key upgrade over ChaCha20-Poly1305:
      ...
      - Safe for random nonce generation at scale
...
class MLKEM:
    ...
    def _pke_keygen(self) -> Tuple[bytes, bytes]:
        ...
        d        = os.urandom(32)
        digest   = hashlib.sha3_512(d).digest()
    ...
    def keygen(self) -> Tuple[bytes, bytes]:
        """
        ML-KEM.KeyGen - returns (encapsulation_key, decapsulation_key).
...
class HybridPQCipher:
    ...
    Security: secure as long as EITHER ML-KEM OR XChaCha20 remains unbroken.
    """
    ...
    def decrypt(self, dk: bytes, bundle: bytes) -> bytes:
        """Decrypt a bundle produced by encrypt(). Raises RuntimeError if tampered."""
...
def run_tests():
    ...
    for level in [512, 768, 1024]:
        ...
        print(f"    ML-KEM-{level} PASSED  (incl. implicit rejection)\n")
    ...
    print(f"  Hybrid PQ PASSED\n")

    print("="*78)
    print("  ALL POST-QUANTUM TESTS PASSED")
